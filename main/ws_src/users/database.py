...
def update_user_product_based_on_order_type(user_product, created, order_dto):
    if order_dto.type == BaseOrderType.SALE:
        if not created:
            increase_user_product_quantity_and_price(user_product, order_dto.quantity, order_dto.transaction_price)
    elif order_dto.type == BaseOrderType.PURCHASE:
        validate_user_product_for_purchase(user_product, order_dto.quantity, order_dto.transaction_price)
        decrease_user_product_quantity_and_price(user_product, order_dto.quantity, order_dto.transaction_price)
